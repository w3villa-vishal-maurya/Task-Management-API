...
async function regReq(req, res, next) {
  try {
    ...
    return res
      ...
      .json({ message: "User Successfully registered!! You can login!" });
  } catch (error) {
...
function logOutReq(req, res, next) {
  if (req.session.autherization["accessToken"]) {
    ...
    return res
      .status(200)
      .json({ title: "Successful", message: "User logged Out!" });
  } else {
    logger.info("User is not loogged In, Login First");
    return res
      .status(StatusCodes.FORBIDDEN)
      .json({ message: "User is not loogged In, Login First" });
  }
}
  ...
  try {
    ...
    if (user.length > 0) {
      ...
      const data = await User.updateOne(
        { email: email },
        { $set: { passResetToken: passResetToken } }
      );
      const isSendEmail = await sendResetPasswordMail(
        user[0].name,
        user[0].email,
        passResetToken
      );
      if (isSendEmail) {
        ...
        throw new BadRequestError("Internal server error Occured!!");
      }
    } else {
      throw new UnauthenticatedError("User is not Registered!");
    }
...
async function resetPassword(req, res, next) {
  try {
    ...
    if (user.length > 0) {
      ...
      const data = await User.updateOne(
        { passResetToken: passResetToken },
        { $set: { password: hashPassword, passResetToken: null } }
      );
      return res
        .status(StatusCodes.CREATED)
        .json({
      ...
      throw new BadRequestError("Generated Token expired or invalid!");
    }
  } catch (error) {
    return next(error);
  }
...
async function getAllUsers(req, res, next) {
  try {
    ...
    let allUser = [];
    if (userName === "") {
      allUser = await User.find({}).select('name email');;
    } else {
      allUser = await User.find({ name: { $regex: userName, $options: "i" } }).select('name');;
    }

    return res
      .status(StatusCodes.OK)
      .json({ title: "Successful", allUser: allUser });
  } catch (error) {
    return next(error);
  }
}
